...
- **`kiruk-portal` — onboarding rebuilt as Absurdly interactions (no more form).** Adopted the founder's **Absurdly** design language (Figma) for the onboarding/tests: graph-paper grid ground, brutalist grotesque type, inverted black blocks, dashed = secondary, the face mark, circle-→ step nav, page dots. Every step is a scene you act on — personality = **big invert-on-pick shape tiles** + a black "THE READ" archetype block; vibe = **chunky pointer-drag gauges** (PASS/FAIL needle energy, poles scale with value); client = split white-label / black-fill inputs. Verified every step in-browser. Caught a `.invert` collision with Tailwind's invert _filter_ utility → renamed `.ink-block`. (Portal = the absurd, playful surface; distinct from kiruk-web's pencil hero.)
  - **Why:** the prior tests read like an old-style form. Immersive, conceptual, creative-technologist interaction is the brand — the read of the client should feel like the studio.
