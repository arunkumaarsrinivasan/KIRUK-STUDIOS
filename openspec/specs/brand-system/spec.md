...
The studio MUST define exactly one primary eye-mark logo as a React SVG component at `packages/design-system/components/eye/EyePrimary.tsx`, with a static SVG export at `packages/design-system/build/logos/primary.svg`.
...
- THEN it MUST import `EyePrimary` from `packages/design-system/components/eye/EyePrimary` (or a documented derivative) and no other mark
...
The brand MUST pair one **rational** typeface (for UI, structure, grids) with one **expressive** typeface (for headings, manifesto voice). Both MUST be declared in `packages/design-system/tokens/type.json`.
