...
export default async function ShippingPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="bg-paper relative min-h-[100svh] w-full px-6 py-10 md:px-10">
      <div className="bg-grid bg-vert" aria-hidden="true" />
      <header className="relative mb-6 flex items-center gap-3">
        ...
        <div className="leading-none">
          <h1 className="brutal text-ink text-4xl">{u.title} — shipping</h1>
          <p className="handwritten text-pencil text-sm">spec.md gates shipping · {slug}</p>
        </div>
      </header>
      ...
      <section className="relative">
        <ShippingPanel slug={u.slug} hasSpec={!!u.artifacts['spec.md']} />
      </section>
    </main>
  );
}
