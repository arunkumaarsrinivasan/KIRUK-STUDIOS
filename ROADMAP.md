...
- [x] Surfaces consuming it — `/dashboard`+todo, `/admin` CRUD, `/client`(+`[slug]`) read-view, `/sign-in` magic-link, landing nav + onboarding→DB. Proven running on pglite (auth gating 307→/sign-in; magic-link issues + persists). **Phase 4 exit criteria met.**
